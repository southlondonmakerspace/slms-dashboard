...
function sensibleEvent(ev) {
    ...
    if (ev.description != undefined) {
        // if there are multiple lines, just get the first line
        if (ev.description.includes("\n")) {
            sensible.description = ev.description.split("\n")[0]
        }
        // and only the first sentence
        if (sensible.description.includes("."))
        {
           sensible.description = sensible.description.split(".")[0]
        }
    }
    return sensible
}
